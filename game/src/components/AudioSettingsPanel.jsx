...
import { applyCaseReaderVolume } from '../lib/caseReader.js';
...
export default function AudioSettingsPanel({ embedded = false, showGameSounds = true }) {
  ...
  const bump = (patch) => {
    ...
    setPrefs(next);
    applyMonitorVolume();
    applyCaseReaderVolume();
    unlockAmbience();
  };

  const monitorPct = Math.round((prefs.monitorVolume || 0) * 100);
  const sfxPct = Math.round((prefs.sfxVolume || 0) * 100);
  const voicePct = Math.round((prefs.voiceVolume || 0) * 100);

  return (
    <section
      className={`audio-settings-panel ${embedded ? 'audio-settings-panel--embedded' : ''}`}
    ...
    >
      ...
      <VolumeRow
        ...
        onMute={() => {
          if (prefs.monitorMuted) {
            bump({
              monitorMuted: false,
              monitorVolume: prefs.monitorVolume > 0 ? prefs.monitorVolume : 0.23,
            });
          } else {
      ...
      <VolumeRow
        label="Voice reading"
        ariaPrefix="voice reading"
        muted={prefs.voiceMuted}
        pct={voicePct}
        onMute={() => {
          if (prefs.voiceMuted) {
            bump({
              voiceMuted: false,
              voiceVolume: prefs.voiceVolume > 0 ? prefs.voiceVolume : 1,
            });
          } else {
            bump({ voiceMuted: true });
          }
        }}
