#!/usr/bin/env node
/**
 * Merge CardioCard-angle layout 8 into bundled user layout.
 * Source: assets/ecg-vector-lab/layouts/cardiocard-angle-layout.json
 * Optional: node scripts/apply-ecg-angle-layout8.mjs "C:\path\to\export.json"
 */
import fs from 'fs';
...
/* Layout 8 export has no heartRotation yet — default 0 for bundled boot */
if (typeof src.heartRotation !== 'number') src.heartRotation = 0;
...
dst.note =
  'Layout 8 — scope/heart on chest, heartScale 1.31. Revert: node scripts/revert-ecg-angle-layout8.mjs';
...
writeJson(angleLayoutPath, {
  ...src,
  ...dst,
  note: dst.note,
  visibleLeads: dst.visibleLeads,
  focusLead: dst.focusLead,
  heartRotation: dst.heartRotation,
});

console.log('Applied layout 8 →', dstPath);
console.log('Updated angle layout →', angleLayoutPath);
console.log('scope:', dst.scope?.x?.toFixed(1), dst.scope?.y?.toFixed(1));
console.log('heartAnchor:', dst.heartAnchor?.x?.toFixed(1), dst.heartAnchor?.y?.toFixed(1));
console.log('heartScale:', dst.heartScale);
