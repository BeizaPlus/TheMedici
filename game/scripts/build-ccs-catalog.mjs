...
import { CCS_PRESENTATIONS_DIR, STEP3_TOOLS_DIR } from './paths.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, '..');
const STEP3_CANDIDATES = [path.join(ROOT, 'step3')];
...
const presDir = [CCS_PRESENTATIONS_DIR, path.join(STEP3_TOOLS_DIR, 'ccs_presentations')].find((p) =>
  fs.existsSync(p),
);
const presentations = loadPresentations(presDir || CCS_PRESENTATIONS_DIR);
