...
import { AttendingDemoSequence } from '../lib/attendingDemoSequence.js';
...
export default function Play({
  ...
  const [demoEngineState, setDemoEngineState] = useState('idle');
  ...
  const onStartDemo = useCallback(() => {
    ...
    const engine = new AttendingDemoSequence({
      ...
      placementOrderLength: placementOrder.length,
      delayMs: 800,
      narrateMs: 30,
      onPlace: async (iv, index) => {
        const dropZone = stackDropZoneForIv(iv, index);
        setPlaced((p) => ({ ...p, [iv.id]: dropZone }));
        setPlacementOrder((prev) => (prev.includes(iv.id) ? prev : [...prev, iv.id]));
        setPins((prev) => [
          ...prev.filter((pin) => pin.ivId !== iv.id && pin.label !== iv.label),
          applyLayoutToPhysicalExamPin({ zoneId: dropZone, label: iv.label, ivId: iv.id, ok: null }),
        ]);
        setStackMoveMode(true);
        setReviewed(false);
        setReviewResults({});
        ...
        setWhyPanel(null);
        setTeachFocusId(null);
        logTimeline({ type: 'stack', stackId: iv.id, label: iv.label, correct: true, method: 'demo' });
        if (teachMeMode) showOrderWhyInDock(iv);
      },
      onNarrate: (text) => setDemoNarration(text),
      onProgress: (step) => setDemoStepIndex(step),
      onComplete: () => {
        setDemoEngineState('completed');
        demoRunningRef.current = false;
        showToast(`Timeline complete \u2014 ${remaining.length} orders`, 'ok');
      },
    });
    ...
    setDemoEngineState('running');
    demoRunningRef.current = true;
    setDemoStepIndex(0);
    setDemoNarration('');
    engine.start();
  }, [interventions, placed, placementOrder.length, logTimeline, teachMeMode, showOrderWhyInDock]);

  const onPauseDemo = useCallback(() => {
    demoEngineRef.current?.pause();
    setDemoEngineState('paused');
    demoRunningRef.current = false;
  }, []);

  const onResumeDemo = useCallback(() => {
    demoEngineRef.current?.resume();
    setDemoEngineState('running');
    demoRunningRef.current = true;
  }, []);

  const onStepBackward = useCallback(() => {
    demoEngineRef.current?.stepPrevious();
    setDemoStepIndex((prev) => Math.max(0, prev - 1));
  }, []);
  ...
  const onStepForward = useCallback(async () => {
    ...
    if (!engine || engine.state === 'completed') {
      const remaining = interventions.filter((iv2) => !placed[iv2.id]);
      if (!remaining.length) return;
      const newEngine = new AttendingDemoSequence({
        interventions: remaining,
        placementOrderLength: placementOrder.length,
        onPlace: placeOne,
        onNarrate: (text) => setDemoNarration(text),
      ...
      demoEngineRef.current = newEngine;
      setDemoEngineState('paused');
      await newEngine.stepNext();
      setDemoEngineState(newEngine.state === 'completed' ? 'completed' : 'paused');
      return;
    }

    setDemoEngineState('paused');
    await engine.stepNext();
    setDemoEngineState(engine.state === 'completed' ? 'completed' : 'paused');
  }, [interventions, placed, placementOrder.length, logTimeline, teachMeMode, showOrderWhyInDock]);
    ...
    >
      ...
      >
        ...
        <div className="scene-dock-left">
          ...
          <div className="scene-dock-wrap">
            {attendingDemoMode && (
              <div className="scene-order-demo-bar">
                {demoNarration && (
                  <div className="scene-order-demo-narration" aria-live="polite">
                    <span className="scene-order-demo-narration-avatar" aria-hidden>👩‍⚕️</span>
                    <span className="scene-order-demo-narration-text">{demoNarration}</span>
                    {demoRunningRef.current && <span className="scene-order-demo-narration-cursor" aria-hidden>|</span>}
                  </div>
                )}
                {remainingDemoStacks.length > 0 && (
                  <div className="scene-order-demo-progress">
                    <span className="scene-order-demo-progress-text">
                      {demoEngineState === 'completed'
                        ? `Complete \u2014 ${remainingDemoStacks.length + demoStepIndex} orders`
                        : `Order ${Math.min(demoStepIndex + 1, remainingDemoStacks.length)}/${remainingDemoStacks.length}`}
                    </span>
                    <div className="scene-order-demo-progress-track">
                      <div
                        className="scene-order-demo-progress-fill"
                        style={{ width: `${(demoStepIndex / Math.max(1, remainingDemoStacks.length)) * 100}%` }}
                      />
                    </div>
                  </div>
                )}
                <div className="scene-order-demo-controls">
                  {demoEngineState === 'running' ? (
                    <button type="button" className="scene-order-demo-control-btn" onClick={onPauseDemo} title="Pause demo" aria-label="Pause demo">
                      <IconPlayerPause aria-hidden />
                    </button>
                  ) : demoEngineState === 'paused' || (demoEngineState === 'idle' && demoStepIndex > 0) ? (
                    <button type="button" className="scene-order-demo-control-btn" onClick={onResumeDemo} title="Resume demo" aria-label="Resume demo">
                      <IconPlayerPlay aria-hidden />
                    </button>
                  ) : (
                    <button type="button" className="scene-order-demo-control-btn" onClick={onStartDemo} disabled={remainingDemoStacks.length === 0 || demoEngineState === 'completed'} title={remainingDemoStacks.length === 0 ? 'All orders placed' : 'Start demo'} aria-label="Start demo">
                      <IconPlayerPlay aria-hidden />
                    </button>
                  )}
                  <button type="button" className="scene-order-demo-control-btn" onClick={onStepBackward} disabled={demoStepIndex <= 0 || demoRunningRef.current} title="Previous step" aria-label="Previous step">
                    <IconSkipBack aria-hidden />
                  </button>
                  <button type="button" className="scene-order-demo-control-btn" onClick={onStepForward} disabled={demoStepIndex >= remainingDemoStacks.length || demoRunningRef.current} title="Next step" aria-label="Next step">
                    <IconSkipForward aria-hidden />
                  </button>
                  <span className="scene-order-demo-status">
                    {demoEngineState === 'running'
                      ? 'Running\u2026'
                      : demoEngineState === 'paused'
                        ? 'Paused'
                        : demoEngineState === 'completed'
                          ? 'Done'
                          : demoStepIndex > 0
                            ? `Step ${demoStepIndex}/${remainingDemoStacks.length}`
                            : `${remainingDemoStacks.length} remaining`}
                  </span>
                </div>
              </div>
            )}
            ...
            demoEngineState={demoEngineState}
