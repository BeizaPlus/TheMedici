#!/usr/bin/env python3
"""Case 099 — ONE Kling i2v from bladder anatomy 2×2 master plate (v3-alt1 approved).

Usage:
  python dev/uber-portrait-refs/run_case099_bladder_2x2_kling.py
  python dev/uber-portrait-refs/run_case099_bladder_2x2_kling.py --duration 10
"""
...
PLATE = GAME / "dev/uber-portrait-refs/game-scenes-pending/Composed_pipe-tweed-mustache-bank-BLADDER-2x2-v3-alt1-20260701.png"
MOTION = GAME / "dev/uber-portrait-refs/prompts/case-099-bladder-2x2-motion.txt"
OUT = GAME / "dev/uber-portrait-refs/video-pending/case-099-bladder-2x2-5s-kling26.mp4"
...
START_NAME = "case099-bladder-2x2-start.jpg"
PREFIX = "video/case099-bladder-2x2"
...
def preflight_comfy() -> None:
    ...
    if running or pending:
        raise SystemExit(
            f"Comfy queue not empty (running={len(running)} pending={len(pending)}).\n"
            f"Wait for current job or restart {KLING26_BAT.name}."
        )
    ...
    while time.time() < deadline:
        ...
        if not entry:
            ...
            if elapsed > ZOMBIE_WARN_SEC:
                raise SystemExit(f"Kling job {prompt_id} stuck after {elapsed}s.")
            print("… waiting")
            time.sleep(12)
            continue
        ...
        print("… rendering bladder 2x2 plate")
        time.sleep(12)
    raise SystemExit(f"Timed out after {timeout}s on {prompt_id}.")
...
def main() -> None:
    ...
    prompt = (
        "16:9 landscape storyboard contact sheet, four panels in fixed 2x2 grid. "
        + motion
    )
    ...
    out = OUT if args.duration <= 5 else OUT.with_name(
        "case-099-bladder-2x2-10s-kling26.mp4"
    )
