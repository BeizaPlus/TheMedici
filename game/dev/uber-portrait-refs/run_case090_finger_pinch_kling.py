#!/usr/bin/env python3
"""Case 090 — ONE Kling i2v from finger-pinch vessel 3×3 master plate (v2, blood flow + hand).

Usage:
  python dev/uber-portrait-refs/run_case090_finger_pinch_kling.py
  python dev/uber-portrait-refs/run_case090_finger_pinch_kling.py --duration 10
"""
...
PLATE = GAME / "dev/uber-portrait-refs/video-pending/case-090-finger-pinch-vessel-storyboard-3x3-16x9-v2.png"
MOTION = GAME / "dev/uber-portrait-refs/prompts/case-090-finger-pinch-vessel-3x3-motion.txt"
OUT = GAME / "dev/uber-portrait-refs/video-pending/case-090-finger-pinch-vessel-5s-kling26.mp4"
...
START_NAME = "case090-finger-pinch-vessel-start.jpg"
PREFIX = "video/case090-finger-pinch-vessel"
...
def poll(prompt_id: str, timeout: int = POLL_TIMEOUT_SEC) -> dict:
    ...
    while time.time() < deadline:
        ...
        print("… rendering finger-pinch vessel plate")
        time.sleep(12)
...
def main() -> None:
    ...
    out = OUT if args.duration <= 5 else OUT.with_name(
        "case-090-finger-pinch-vessel-10s-kling26.mp4"
    )
