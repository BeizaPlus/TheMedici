...
DATA_DIR = ROOT / "game" / "data" / "ollama"
...
ANSWER_KEY_OCR_PROMPT = """You are a strict clinical data extractor.

You will receive OCR text from a CCS (Clinical Case Simulation) screenshot.

YOUR ONLY JOB: extract what is literally present in the OCR text.

ABSOLUTE RULES:
- If a value is not explicitly in the OCR text, return null. No exceptions.
- Do NOT guess, infer, hallucinate, or pull from medical knowledge.
- Do NOT fill fields with content from other cases or conditions.
- Do NOT write generic medical text.
- If the OCR text is unclear or garbled, return null for that field.
- The title must match exactly what the screenshot says.
- The stacks must be exactly what appears in the screenshot stack cards — nothing added.
- If you are not certain a word appears in the OCR, do not include it.
...
For stacks — each item must be an object:
{
  "label": "exact text from the screenshot stack card",
  "aliases": [],
  "finding": null,
  "type": null
}

For hpi_narrative — ONLY populate if hpi_raw has real content.
If populated, rewrite hpi_raw as a clinical presentation in this exact format:
"[Ms/Mr] X is a [age]-year-old [sex] with [relevant PMH or 'no significant past medical history']
who presents with [chief complaint]. [Symptom onset, character, location, severity, duration].
[Associated symptoms]. [Pertinent negatives]. [Key objective finding on arrival if present]."

If hpi_raw is null or garbled, hpi_narrative must also be null.

SELF-CHECK before returning:
- Does the title match the screenshot exactly?
- Is every stack label copied verbatim from the screenshot?
- Is there any content I added that was not in the OCR? If yes, remove it.
"""
...
def phase_answer_keys(args, extract_mod) -> None:
    ...
    for case_id in targets:
        ...
        if ocr is None:
            ...
            extract_mod.write_structured_case_json(raw, case_id, ocr=ocr, title=title)
